...
**Uploads, acknowledgements, requeues and secrets, as built.** A task redeems its grant once, after the image pull, and gets one signed POST policy for its outputs; the built-in store, at `POST /objects/{ns}`, holds each object to its digest, which MinIO and S3 cannot, and the page says so. A runner records a key under its work root and acknowledges on take, and the page names the one gap left: a host that dies between the acknowledgement and the redemption leaves the task to the run's timeout. A lost task is requeued under its key with a new `task_id` only where `retry.on` names `lost` and the step is idempotent, spending no retry and waiting no backoff. A result travels on its runner's own subject and is heard only from the runner its dispatch is bound to. A namespace declares its secrets through `/api/v1/{ns}/secrets/{name}`, under a ninth permission, `secret:write`: `builtin` with a write-only value, or `env` under an `AGK_DEV_` prefix the installation opts in; `vault` is refused until v0.9.0. A workflow only names them, `secrets: [billing]`, and a mount may now carry a dot, `/agk/secrets/client.key`. On the roadmap, suspending a waiting run joins the wait step in v0.8.0, and Vault leaves v0.2.0.
