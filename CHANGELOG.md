...
**Uploads, acknowledgements, requeues and secrets, settled.** A task's outputs are written under one signed POST policy, handed out at the redemption. The built-in store, which no deployment profile uses, hashes what it takes; MinIO and S3 cannot, and the page says so rather than promise it. A runner acknowledges a task on take, once it has recorded the key and redeemed the grant, so the API knows who holds every task the bus has let go of, and refuses a key its host has completed. A lost task whose retry policy names `lost` and whose step is idempotent is requeued under its key with a new `task_id`, spends no retry and waits no backoff, and a heartbeat or a result speaks only for the dispatch its runner holds, so a result from a runner its `task_id` was not bound to is refused. A namespace declares its secrets (`builtin`, or `env` or `vault` at a path under a prefix the installation gives it, which no path may climb out of) under a ninth permission, `secret:write`, which, like reading the declarations, counts only at namespace scope; a workflow only names them, `secrets: [billing]`. A `builtin` value goes with its declaration, so moving one to another provider cannot leave a value to come back later. On the roadmap, suspending a waiting run joins the wait step in v0.8.0, and Vault leaves v0.2.0.
