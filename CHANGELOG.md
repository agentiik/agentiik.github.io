...
**Requeues bounded, and a run cancelled, as built.** A host answers the requeue of a key it already ended with the ending it recorded, by reference, so the brick never runs twice. The sweep declares lost a task nobody redeemed 30 seconds after its publication, and `max_requeues`, an installation setting of 3 per key by default, bounds requeues: past it the step fails on the infrastructure's account. `POST /api/v1/runs/{id}/cancel` is written down as built, and the roadmap keeps cancel in v0.2.0 while approve and reject join the wait step in v0.8.0. Each route's body limit is listed, a redemption binds only once it can answer, and an identifier is at most 255 characters. A message the bus hands out again after its AckWait redeems nothing, since the sweep has ended its task by then. The Task bus, the dispatched state, the hooks and profile C say what the sweep costs a task waiting on a full queue or a slow pull: its step fails at the first loss unless it requeues on loss.
